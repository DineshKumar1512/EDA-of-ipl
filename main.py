...
k=df5['venue'].value_counts().nlargest(20)
df56=df[df['toss_decision']=='field']
l=df56['venue'].value_counts().nlargest(20)
m=df.groupby('venue')['winner'].count().sort_values(ascending=False)
z = st.selectbox('data type', ['select','Match Winner','Season','Venue','City',
'Toss Winner','Result','Man of the Match','Toss Decision','umpire',
'IPL Trophies','Batting Venue','Bowling Venue','Winning Venue'],key='asd')
select = st.selectbox('Visualization type', ['select','Bar chart', 'Area chart',
'Scatter chart','Pie chart'],key='asd')
if z=='Match Winner':
    y=a
elif z=='Season':
    y=b
...
if select == 'Area chart':
    ...
    fig=px.bar(y,x=y.index,y=y.values,color=y.values,
    color_continuous_scale=["#003f5c","#bc5090","#ffa600"],height=500)
    st.plotly_chart(fig)
elif select=='Scatter chart':
    fig=px.scatter(y,x=y.index,y=y.values,color=y.values,
color_continuous_scale=["#003f5c","#bc5090","#ffa600"],height=500)
    st.plotly_chart(fig)
...
d1=df1.groupby('bowler')['player_dismissed'].count()
...
y2=y[y>30]
g1=((y2/z2)).sort_values(ascending=True).head(10)
...
y4=y[y>1000]
i1=((y4/z4)).sort_values(ascending=False).head(10)

z=df1.groupby('bowler')['total_runs'].sum().sort_values(ascending=False)
z6=z[z>500]
y=df1['bowler'].value_counts()
y6=(y[y>300])
n1=((z6/y6)*6).sort_values(ascending=True).nlargest(10)
...
z1 = st.selectbox('DATA TYPE', ['select','Dismissal Type','CAtch taken by','Most Runs',
'Most wickets','Balls Faced','Batsman SR','Bowling SR','Batting Average',
'Bowling Average','Bowling Economy','Most Sixes','Most Fours','Most Ones','Most Dots'])
select1 = st.selectbox('VISUALIZATION TYPE', ['select','Bar chart', 'Area chart',
...
if select1 == 'Area chart':
    ...
    fig=px.scatter(y1,x=y1.index,y=y1.values,color=y1.values,color_continuous_scale=["red","yellow"],height=500)
    st.plotly_chart(fig)
...
b2=merge_df.groupby('winner')['player_dismissed'].count().sort_values(ascending=False)
c2=merge_df.groupby('season')['batsman_runs'].sum().sort_values(ascending=False)
d2=merge_df.groupby('season')['player_dismissed'].count()
e2=merge_df.groupby('venue')['batsman_runs'].sum().sort_values(ascending=False)
f2=merge_df.groupby('venue')['player_dismissed'].count().sort_values(ascending=False)

z2 = st.selectbox('data type', ['select','Winning Team Runs','Winning Team Wickets',
'Runs per season','Wickets per season','Batting stadiums','Bowling stadiums'],key='asdf')                               
select2 = st.selectbox('Visualization type', ['select','Bar chart', 'Area chart','Scatter chart',
'Pie chart'],key='asdf')
...
if select2 == 'Area chart':
    ...
    fig=px.bar(y2,x=y2.index,y=y2.values,color=y2.values,color_continuous_scale=
["#004c6d","#00a1c1","#00ffff"],height=500)
    st.plotly_chart(fig)
elif select2=='Scatter chart':
    fig=px.scatter(y2,x=y2.index,y=y2.values,color=y2.values,color_continuous_scale=
["#004c6d","#00a1c1","#00ffff"],height=500)
    st.plotly_chart(fig)
...
st.write(x4)
z3 = st.selectbox('data type', ['select','Batting SR season','Bowling SR season','Batting average season','Bowling average season','Bowling economy season'],key='asdfgh')                               
...
y12=y11[y11>50]
b3=((y12/z12)).sort_values(ascending=False).head(10)
...
if select3 == 'Area chart':
    ...
    fig=px.bar(y3,x=y3.index,y=y3.values,color=y3.values,color_continuous_scale=
["#FFC300","#FF5733","#C70039","#900C3F"],height=500)
    st.plotly_chart(fig)
elif select3=='Scatter chart':
    fig=px.scatter(y3,x=y3.index,y=y3.values,color=y3.values,color_continuous_scale=
    ["#FFC300","#FF5733","#C70039","#900C3F"],height=500)
    st.plotly_chart(fig)
